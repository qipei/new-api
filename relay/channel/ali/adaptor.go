...
func isAliUnifiedImageModel(modelName string) bool {
	modelName = strings.ToLower(strings.TrimSpace(modelName))
	return strings.HasPrefix(modelName, "vidu/") || strings.HasPrefix(modelName, "kling/")
}
...
func (a *Adaptor) ConvertImageRequest(c *gin.Context, info *relaycommon.RelayInfo, request dto.ImageRequest) (any, error) {
	if info.RelayMode == constant.RelayModeImagesGenerations {
		...
		if isAliUnifiedImageModel(info.UpstreamModelName) {
			...
			aliRequest, err := oaiImage2AliImageRequest(info, request, false)
			if err != nil {
				return nil, fmt.Errorf("convert image request to ali unified image request failed: %w", err)
			}
			return aliRequest, nil
		}
