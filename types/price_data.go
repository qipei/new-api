...
type PriceData struct {
	FreeModel  bool
	ModelPrice float64
	// FixedPrice is charged once per request and is not multiplied by output
	// count ratios. Image matrix input-image and prompt-token charges use it.
	FixedPrice           float64
	ModelRatio           float64
...
func (p *PriceData) ToSetting() string {
	return fmt.Sprintf("ModelPrice: %f, FixedPrice: %f, ModelRatio: %f, CompletionRatio: %f, CacheRatio: %f, GroupRatio: %f, UsePrice: %t, CacheCreationRatio: %f, CacheCreation5mRatio: %f, CacheCreation1hRatio: %f, QuotaToPreConsume: %d, ImageRatio: %f, AudioRatio: %f, AudioCompletionRatio: %f", p.ModelPrice, p.FixedPrice, p.ModelRatio, p.CompletionRatio, p.CacheRatio, p.GroupRatioInfo.GroupRatio, p.UsePrice, p.CacheCreationRatio, p.CacheCreation5mRatio, p.CacheCreation1hRatio, p.QuotaToPreConsume, p.ImageRatio, p.AudioRatio, p.AudioCompletionRatio)
}
